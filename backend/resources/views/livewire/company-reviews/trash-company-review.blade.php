<div class="container-fluid px-3 px-md-4 py-3 bg-body text-body">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <h4 class="fw-semibold mb-0">
            <i class="fa-solid fa-trash-can me-2 text-danger"></i> Deleted Company Reviews
        </h4>
        <a wire:navigate href="{{ route('company-reviews.index') }}" class="btn btn-outline-secondary">
    ...
    <div class="card shadow-sm border border-body bg-body text-body rounded-3">
        <div class="card-body p-0">
            @if($trashedReviews->isEmpty())
                <div class="text-center py-5">
                    <i class="fa-solid fa-trash-can text-secondary mb-3" style="font-size: 3rem;"></i>
                    <p class="text-secondary mb-0">No deleted reviews found</p>
                </div>
            @else
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead class="border-bottom">
                            <tr>
                                <th scope="col" class="text-nowrap py-3">Review</th>
                                <th scope="col" class="text-nowrap">Rating</th>
                                <th scope="col" class="text-nowrap">Candidate</th>
                        ...
                        <tbody>
                            @foreach($trashedReviews as $review)
                                <tr>
                                    <td class="text-nowrap">{{ $review->review ?? '-' }}</td>
                                    <td class="text-nowrap">{{ $review->rating }}/5</td>
                                    <td class="text-nowrap">{{ $review->candidate->name ?? '-' }}</td>
                                    <td class="text-nowrap">{{ $review->company->name ?? '-' }}</td>
                                    <td class="text-nowrap">{{ $review->deleted_at->diffForHumans() }}</td>
                                    <td class="text-nowrap text-end">
                                        <div class="d-flex flex-wrap justify-content-end gap-2">
                                            <button type="button"
                                                    onclick="confirmRestore({{ $review->id }})"
