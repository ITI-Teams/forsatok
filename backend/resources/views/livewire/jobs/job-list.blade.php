<div class="container-fluid px-3 px-md-4 py-3 bg-body text-body">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            <livewire:search.search :search-fields="['title','experience']" emit-event="jobSearchUpdated" placeholder="Search jobs..." />
            @can('jobs.manage')
                ...
                <table class="table align-middle mb-0 table-hover">
                    ...
                    @forelse($jobs as $job)
                        <tr>
                            ...
                            <td><span class="badge bg-info text-white">{{ ucfirst($job->work_type ?? 'N/A') }}</span></td>
                            ...
                            <td>
                                @if($job->location)
                                    ...
                                    @if($job->location->address)
                                        <br><small style="color: var(--bs-secondary-color)">{{ $job->location->address }}</small>
                                    @endif
                                @else
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    @can('jobs.manage')
                                        <a wire:navigate href="{{ route('jobs.show', $job->id) }}" class="btn btn-sm btn-info text-white">
                                            <i class="fa-solid fa-eye"></i>
                                        </a>
                                        <a wire:navigate href="{{ route('jobs.edit', $job->id) }}" class="btn btn-sm btn-warning">
                                    ...
                                    @can('jobs.approve')
                                        @if($job->is_active)
                                            <button wire:click="openApprovalModal({{ $job->id }})" class="btn btn-sm btn-danger text-white">
                                                <i class="fa-solid fa-x"></i>
                                            </button>
                                        @else
                                            <button wire:click="openApprovalModal({{ $job->id }})" class="btn btn-sm btn-success text-white">
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Are you sure you want to Active or Inactive the job:
                        <strong>{{ $selectedJob->title }}</strong>?
                    </div>
                    <div class="modal-footer">
                        <button wire:click="approveJob({{ $selectedJob->id }})" class="btn btn-success">Active</button>
                        <button wire:click="rejectJob({{ $selectedJob->id }})" class="btn btn-danger">Inactive</button>
