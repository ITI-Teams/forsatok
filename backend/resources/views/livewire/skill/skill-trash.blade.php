<div class="container-fluid px-3 px-md-4 py-3 bg-body text-body">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <h4 class="fw-semibold mb-0">
            <i class="fa-solid fa-trash-can me-2 text-danger"></i> Deleted Skills
        </h4>
        <a wire:navigate href="{{ route('skills.index') }}" class="btn btn-outline-secondary">
            <i class="fa-solid fa-arrow-left me-2"></i> Back to Skills
        </a>
    </div>

    <div class="card shadow-sm border border-body bg-body text-body rounded-3">
        <div class="card-body p-0">
            @if($trashedSkills->isEmpty())
                <div class="text-center py-5">
                    <i class="fa-solid fa-trash-can text-secondary mb-3" style="font-size: 3rem;"></i>
                    <p class="text-secondary mb-0">No deleted skills found</p>
                </div>
            @else
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @foreach($trashedSkills as $skill)
                                <tr>
                                    <td class="text-nowrap">{{ $skill->name }}</td>
                                    <td class="text-nowrap">{{ $skill->deleted_at->diffForHumans() }}</td>
                                    <td class="text-nowrap text-end">
                                        <div class="d-flex flex-wrap justify-content-end gap-2">
                                            ...
                                            <button type="button"
                                                    onclick="confirmSkillForceDelete({{ $skill->id }})"
